...
##got the wrong dterministic function
...
score = mean(c(abs(a-a2),abs(b-b2),abs(sd-sd2)))
score  
...
### got the wrong deterministic function

#note that this is very sensitive to starting values
#a2=
#b2=
  
#score=mean(abs(a-a2),abs(b-b2))  
```
...
```{r}
myData <- read.csv('dodo_population1.csv')
...
alpha2 = 100 
  ...
  score=mean(c(abs(alpha-alpha2), abs(beta-beta2), abs(sd2-sd)))
  score
```
...
#got the model wrong (jenna hyrnes group)

# Sarah + Narda team. Got poisson? BUt then Normal and power law.
