...
I used a mystery model to simulate the data saved in the "mysteryData.csv" 
file in the homework folder on D2L.
...
plot(mysteryData$predictor, residuals)
abline(h=0, lty=2)
```



2) Copy and edit the code above and change the deterministic function (myDet) to be an exponential function ($a \times x^{b}$). 
Hint: you will also have to change the line of code under "## use the model to generate data"
Does this new model do a better or worse job of fitting the data than the first model? Explain your answer based on the plots you have generated.
