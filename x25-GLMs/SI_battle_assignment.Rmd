...
### Simulation
 ...
 - and a **deterministic function** from the following list:
 	  1. linear
 	  ```{r}
 	  detFunc <- function(x,a,b){return(a + b*x)}
 	  ```
 	  2. exponential decay 
 	  ```{r}
 	  detFunc <- function(x,a,b){return(a*exp(-b*x))}
 	  ```
 	  3. logistic
 	  ```{r}
 	  detFunc<- function(x,a,b){return(exp(a+b*x)/(1+exp(a+b*x)))}
    ```
 	  4. power-law
    ```{r}
    detFunc <- function(x,a,b){return(a*x^b)}
    ```
 ...
 - **Make data files for the other team**:
      1. make a **.csv file** containing 2 columns, labeled "predictor" and "response".
      2. a **realistic and well-motivated writeup** in a word doc of a biological scenario 
 ...
 - **Make an Rmarkdown document** that contains: 
      1. the code used to simulate the data 
  	  2. a demonstration that you are able to recover your chosen parameters using mle2()
...
### Timeline
 ...
 - Dec 1 - make groups, start to plan

 - Dec 1-4 - work within groups to simulate data
 
 - Dec 5 (Monday) - by 9 am, email simulation scenario to me and [TA] to get the green light
 
 	+ teams with problems in their simulation code/scenario will have until 12/5 at 5pm to get their simulations cleared by instructors

 - Dec 6 - data handoff between teams at 9am (beginning of class)

 - Dec 8 - presentations
 
 	+ **Presentations must be submitted to the D2L dropbox by by 9am on Dec 8 ** 