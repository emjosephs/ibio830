---
title: "Lecture 25 - Model Building continued: GLMs and link functions"
...
So far, we've just used the Normal distribution, 
but we can **D.R.E.A.M.** bigger!
...
How does leaf size affect wait time to first mite?
...
## Model building: recap {.build}
 ...
 - we can do this with any deterministic function and any probability distribution
 
## Model building: D.R.E.A.M. bigger!

```{r, out.width="700px",echo=FALSE,fig.align="center"}
	knitr::include_graphics("figs/dream.001.jpeg")
```

## Model building: D.R.E.A.M. bigger!

```{r, out.width="700px",echo=FALSE,fig.align="center"}
	knitr::include_graphics("figs/dream.002.jpeg")
```
...
par(mar=c(4,4,1,1))
plot(plant.height,occ.lfs/30,pch=20,
	col=adjustcolor(1,0.3),ylim=c(0,1),
	xlab="plant height",
	ylab="proportion of censused leaves with mites",
	main="")
lines(plant.height[order(plant.height)],
		inv.logit.link(mod@coef[1] + 
						mod@coef[2] * plant.height)[order(plant.height)],				
		col=2,lwd=3)
	text(x=1.5,y=0.8,labels=sprintf("a = %s\nb = %s",round(mod@coef[1],2),round(mod@coef[2],2)),cex=1.5)
	legend(x="bottomright",lty=1,lwd=3,col=2,legend="model fit")
```
...
## Another example of GLMs: fungus-induced mortality

- When fungal outbreaks get particularly severe, 
they can kill the whole plant

```{r, out.width="500px",echo=FALSE,fig.align="center"}
	knitr::include_graphics("figs/powdery_mildew.jpeg")
```

## Fungus-induced mortality

 - For many different vineyards, each containing thousands of plants, 
 measure the mean dry fungal weight per plant 2 weeks into the growing season
 
 - At the end of the growing season, count number of dead plants per plot
 	+ assume plots are the same size
...
plot(dry.wt.fungus,mortality,pch=20,col=adjustcolor(1,0.5),
	xlab="mean dry weight fungus",ylab="number of dead plants",
	main="Fungus-induced mortality")
```

## Fungus-induced mortality {.build}

 - How are these data distributed?
 	+ number of dead plants
 	+ [hint: rare event with large sample size...]

$\large y \sim \text{Poisson}(\lambda)$

\

$\large y_i \sim \text{Poisson}(\lambda_i = f(x_i))$
...
$\Large f(\lambda_i) = \beta x_i + \alpha$
\

$\Large
\begin{aligned}
log(\lambda_i) &= \beta x_i + \alpha \\
\lambda_i &= e^{\beta x_i + \alpha}
\end{aligned}$
...
# run a poisson regression with mle2()
...
plot(dry.wt.fungus,mortality,pch=20,col=adjustcolor(1,0.5),
	xlab="mean dry weight fungus",ylab="number of dead plants",
	main="Fungus-induced mortality")
	text(x=5,y=4,labels=sprintf("a = %s\nb = %s",round(mod@coef[1],2),round(mod@coef[2],2)),cex=2)
	lines(seq(0,35,length.out=1000),
			exp(mod@coef[1] + mod@coef[2]*seq(0,35,length.out=1000)),
			col="blue",lwd=3)
```

## Link functions {.build}

Different distributions are commonly associated with different link functions
