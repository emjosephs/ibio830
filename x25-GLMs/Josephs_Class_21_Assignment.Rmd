...
```{r}
plot(mydata$dry.wt.fungus, mydata$mortality)
...
We have count data that ranges from 0 to 6.
...
mod <- mle2(mortality ~ dpois(lambda=invLink( detFunc(dry.wt.fungus,a,b))),
            data=mydata,
            start=list("a"=0,"b"=0))
...
```{r}
myresiduals = mydata$mortality-modData
plot(mydata$dry.wt.fungus, myresiduals)
abline(h=0, lty=2)
```
