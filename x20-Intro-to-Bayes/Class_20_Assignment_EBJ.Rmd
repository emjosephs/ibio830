...
#### Due by the beginning of next class (11/15)
...
1. Write down Bayes' Theorem using $D$ to denote "data" and $H$ to denote "the hypothesis"

$\Large p(H \mid D) = \frac{p(D \mid H)p(H)}{p(D)}$
...
$p(H \mid D)$ is the posterior probability of the hypothesis 

$p(D \mid H)$ is the likelihood of the data given the hypothesis
...
= Number of detected cases x time a case is infections x number of cases per detected case
```{r}
pRSV = (5*7*3)/7000
pRSV
```
