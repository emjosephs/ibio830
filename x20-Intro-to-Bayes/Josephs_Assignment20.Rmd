...
<<<<<<< HEAD
We're adding because this is the log likelihood, not the likelihood.
=======
It's different in that it uses the log likelihood instead of the likelihood, so you add the two components of the denominator instead of multiplying them.
>>>>>>> cd52ea43e102bbb81aed4dca08ebd5b9e90b0ad2
...
#calculate-likelihoods for the grid of p
	lnL1 <- sapply(p.seq,function(p){lnL.foo(flips1,n=100,p=p)})
...
The posterior distribution is narrower because we have gotten more data to inform the likelihood, so it ends up mattering more for the posterior than the prior did.
=======
The posterior distribution is narrower because we have more data points in the calculation of the likelihood.
>>>>>>> cd52ea43e102bbb81aed4dca08ebd5b9e90b0ad2
