...
prior: $p(\text{rona})$ = (new daily cases * number of days infectious * number of unreported cases per reported case)/population of Ingham County = `r 4*100*10/280000`\

```{r, echo=FALSE}
priorRona <- 4*100*10/280000
sensitivity <- 0.917
specificity <- 0.989
...
likelihood: $p(\text{positive result} \mid \text{infected with SARS-CoV-2})$ = `r sensitivity`\

marginal likelihood: $p(\text{positive result}) = p(\text{rona})p(\text{positive result} \mid \text{rona}) + (1-p(\text{rona}))p(\text{positive result} \mid \text{no rona}) = 
`r priorRona*sensitivity + (1-priorRona)*falsePos`$

posterior probability: $p(\text{rona} \mid \text{positive result}) = \frac{p(\text{positive result} \mid \text{rona}) \times p(\text{rona})}{p(\text{positive result})}$

$\Large p(\text{rona} \mid \text{positive result}) =  `r (sensitivity * priorRona)/(priorRona*sensitivity + (1-priorRona)*falsePos)`$

So there is only a 49.4% chance that, given your positive antigen test result, you are actually infected with SARS-CoV-2.
...
haveRona <- function(inghamRona,sensitivity,specificity){
	pHaveRona <- (sensitivity * inghamRona) / (inghamRona * sensitivity + (1-inghamRona) * (1-specificity))
	return(pHaveRona)
}

inghamRona <- seq(0,0.1,length.out=1000)
ronaProbs <- haveRona(inghamRona=inghamRona,
	 		   		  sensitivity=0.917,
	 		   		  specificity=0.989)
plot(inghamRona,ronaProbs,
	 xlab="background rate of SARS-CoV-2 in Ingham County",
	 ylab="probability you are actually infected SARS-CoV-2\n given your positive test result")
...
```{r}
inghamRona[which(ronaProbs>0.9)][1]
```
