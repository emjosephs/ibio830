...
Step 8: say something _quantitative_ and _objective_ about 
the effect of a predictor on a response.

Step 7: run model (use likelihood-based 
inference and an algorithm to estimate values of parameters of the model)

Step 6: build **Bayesian** model (make choices about how we think our data are distributed 
and the nature of the relationship between predictor and response)

Step 5: know about prob. distributions and deterministic functions 

Step 4: **learn about Bayes Theorem**
...
Say we have some hypothesis **$\theta$**, often expressed as the belief a parameter takes a particular value 
(e.g., that the probability that a given coin flips Heads is 0.4)

And we also have some observed data **$D$** (e.g., some number of flips from that coin)
...
$\Large{p(A \cap B) = p(A \; \mid \; B) \; \times \; p(B)}$

$\Large{p(B \cap A) = p(B \; \mid \; A) \; \times \; p(A)}$

$\Large{p(A \cap B) = p(B \cap A)}$
