...
- Using data collected in 1975, we know that the proportion of people that develop cancer is 0.01.

- If someone with cancer gets tested, there is a 90% chance the test will find it.

- The probability of a “false positive” where the test saying there was cancer when there wasn't is 0.001.
...
We break down all positive results into 1) positive results if cancer
and 2) positive results if no cancer.
...
- Using data collected in 1975, we know that the proportion of people with cancer is 0.01.

    + this gives us the **prior probability** of developing cancer
...
$p(\text{PosRes} \; \mid \; \text{Cancer}) = 0.9$

$p(\text{Cancer}) = 0.01$

$p(\text{noCancer}) = 1 - 0.01 = 0.99$ 

$p(\text{PosRes} \mid \text{noCancer}) = 0.001$
...
$p(\text{noCancer}) = 1 - 10^{-4} = 0.9999$ 

$p(\text{PosRes} \mid \text{noCancer}) = 0.001$

$p(\text{PosRes}) = 0.01 \times 0.9 + 0.99 \times 0.001 = 0.00999$ 
...
$p(\text{Cancer} \; \mid \; \text{PosRes}) = \frac{9 \times 10^{-5}}{0.00109} = 0.09$
...
Out of 100,000 people: 
 ...
 - there will be 1000 cancer victims
 
	 + of whom 900 will be correctly diagnosed if everyone is tested.

Of the remaining 99,000 healthy people:
 
 - there will be approx 100 false positives.
 
 So if there's a positive diagnosis, the probability that a person actually has cancer is $\frac{90}{90+100}$
