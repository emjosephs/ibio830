...
The probability of the first snow of the year happening on November 15 and November 16.
 
 - non-exclusive events
 
The probability of the first snow of the year happening on November 15 and the probability of it snowing on Christmas.
 
 - conditional probability
 
 \
The probability of it snowing in East Lansing conditional on it being June.
...
```{r}
sampleFunc2 <- function(n){
  sample(1:7, size=n, replace=T)
}

mySampleDays <- sampleDay(1e5)

mySuccess = (mySamples==1)*(mySampleDays==1)
sum(mySuccess)/365
...
```{r}
birthdayFunction <- function(n){
  myBirthdays <- sample(1:365, size=n, replace=T)
  if sum(duplicated(myBirthdays))>0{return(TRUE)} #there's a birthday twin!
  else {return(FALSE)}
}
```
\
