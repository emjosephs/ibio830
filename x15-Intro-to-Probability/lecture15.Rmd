...
I am Gideon Bradburd

- interested in: evolution, population genetics, statistics, coding, natural history

## How I use R -- for work

- R packages, figures, calculator, simulator, intuition builder

## How I use R -- for fun {.build .smaller}

- choose an episode of Star Trek: The Next Generation to watch AT RANDOM!

```{r}
choose.TNG <- function(season=NULL){
	tng.ep.wiki <- paste0("https://en.wikipedia.org/wiki/",
						  "List_of_Star_Trek:_The_Next_Generation_episodes")
	ep.names <- rvest::html_text(
					rvest::html_nodes(
						xml2::read_html(tng.ep.wiki),
						".summary"))
	ep.names <- gsub("\"","",ep.names)
	if(is.null(season)){
		season <- sample(1:7,1)
	}
	n.eps <- c(25,22,26,26,26,26,25)
...
## How I use R -- for fun {.build}
### Anniversary card for your pRtner!

```{r,eval=FALSE}
make.heart <- function(x,y,s){
		z <- seq(0, 2*pi, by=0.1)
		x.heart <- 16/s * sin(z)^3 + x
		y.heart <- (13*cos(z)-5*cos(2*z)-2*cos(3*z)-cos(4*z))/s + y
		return(cbind(x.heart,y.heart))
	}
```
...
hearts <- lapply(1:1000,
				function(n){
				make.heart(x=runif(1,-100,100),
						   y=runif(1,-62.5,62.5),
						   s=15*rbeta(1,5,5))})
				
cols <- rgb(1,0,0,alpha=runif(1000))

animation::saveGIF({
	for(i in 1:50){
		plot(0,xlim=c(-100,100),ylim=c(-62.5,62.5),
			 type='n',xaxt='n',yaxt='n',xlab="",ylab="",bty="n")
		lapply(1:(i*20),
			function(n){
				polygon(hearts[[n]],col = cols[n])
		})
	}
},movie.name="~/Dropbox/TEACHING/IBIO830/EEBstats_2021/Lectures/x15_Intro_to_Probability_1/figs/valengif.gif",
  clean=TRUE,verbose=FALSE,
  ani.height=500,ani.width=800,
  interval=0.05,loop=1,autoplay=FALSE)
```

<iframe src="figs/valengif.gif" height="500px" width="800px" align="middle" style="border: none;"></iframe>


## How I use probability -- for work

> - model estimation with likelihood-based inference
> - models implemented in R packages
	- BEDASSLE, SpaceMix, and conStruct

## How I use probability -- for life
...
```{r, out.width="750px",echo=FALSE,fig.align="center"}
	knitr::include_graphics("figs/not_stat_bio.jpeg")
```

## Gnar 'Zards

```{r, out.width="350px",echo=FALSE,fig.align="center"}
	knitr::include_graphics("figs/bradburd_chuckwallas.jpg")
```


## Need to do $\hspace{3.2cm}$      1st semester goals {.columns-2}
...
```{r,out.width="750px",echo=FALSE,fig.align="center"}
	par(mar=c(5,6,1,1))
	nx <- 105
	plot(x = seq(0,1,length.out=nx),
	     y = -1/(1+exp(-(seq(-30,30,length.out=nx)))),type='l',
	     lwd=5,yaxt='n',xaxt='n',xlab="",ylab="")
	     mtext(side=1,text="time",cex=3,line=2)
	     mtext(side=2,text="Enjoyment\nin course",cex=3,line=0.5)
```
...
```{r,out.width="750px",echo=FALSE,fig.align="center"}
	par(mar=c(5,6,1,1))
	dl <- geosphere::daylength(42.7370,240:345)
	dlCols <- colorRampPalette(c("yellow","darkblue"),nx)
	plot(x = seq(0,1,length.out=nx),
	     y = -1/(1+exp(-(seq(-30,30,length.out=nx)))),type='l',
	     lwd=5,yaxt='n',xaxt='n',xlab="",ylab="")
	     mtext(side=1,text="time",cex=3,line=2)
	     mtext(side=2,text="Enjoyment\nin course",cex=3,line=0.5)
	points(x = seq(0,1,length.out=nx),
	       y = -1/(1+exp(-(seq(-30,30,length.out=nx)))),pch=19,cex=1.5,
	       col=dlCols(nx))
	legend(x="topright",title="hrs daylight",
			pch=19,cex=2.5,legend=c(round(dl[1],1),
								  round(dl[length(dl)],1)),
			col=c(dlCols(nx)[1],dlCols(nx)[nx]))
```
...
```{r,out.width="750px",echo=FALSE,fig.align="center"}
	...
	points(x = seq(0,1,length.out=nx),
	       y = -1/(1+exp(-(seq(-30,30,length.out=nx)))),pch=19,cex=1.5,
	       col=dlCols(nx))
	text(x=0.2,y=-0.5,labels="Marge",cex=3)
	text(x=0.8,y=-0.5,labels="Gideon",cex=3)
```
...
```{r,out.width="750px",echo=FALSE,fig.align="center"}
	...
	points(x = seq(0,1,length.out=nx),
	       y = -1/(1+exp(-(seq(-30,30,length.out=nx)))),pch=19,cex=1.5,
	       col=dlCols(nx))
	text(x=0.2,y=-0.5,labels="Just R",cex=3)
	text(x=0.8,y=-0.5,labels="R & Stats",cex=3)
```
...
This portion of the course will, _by design and necessity_, feel like it moves a little faster.
...
These Times, the coming darkness, etc.
...
In part because the patterns are so subtle and complex, 
...
to use a large-scale simulation to 
estimate the probability of picking a 5 in the following vector:
...
## Probability - some terminology {.verybig .build}
...
<div class ="centered">
$\Huge p(\text{drawing a }5) = \frac{1}{5}$
</div>
...
What's the probability of drawing a 4 out of c(1,2,3,4,5)?
...
\
<div class ="centered">
$\Huge \sum\limits_{i=1}^n p(e_{i}) = 1$
</div>
...
Well, we can get a 6 with:

	{(1,5),(2,4),(3,3),(4,2),(5,1)}
	
and the total *state space* is 5 $\times$ 5 outcomes, 
...
## Quick recap: rules of probability so far
...
## Probability: let's put on our sparkly shoes {.verybig .build}
...
## Daycare example: simple event {.verybig .build}


`r knitr::kable(rowSums(hair_shoe),align="c",col.names="number of kids")`
 
 \
 
 What's the probability that a randomly selected kid 
 has black hair?

## Daycare example: simple event {.verybig}
...
\
 ...
 `r knitr::kable(hair_shoe[,3]/hair_shoe[3,3],align="c",col.names=c("proportion of kids"))`
...
$\begin{align} p(\text{black hair}) = \: &p(\text{black hair AND plain shoes}) \: +\\ &p(\text{black hair AND sparkly shoes}) \end{align}$


## Daycare example: shared event

What's the probability of having black hair AND sparkly shoes?
...
`r knitr::kable(hair_shoe[1:2,1:2]/hair_shoe[3,3],align="c")`

\

The probability of a **shared event** is the product 
of the probabilities of the constitutive simple events, 
so long as the events are independent.


<!-- 
## Daycare example: shared event

`r knitr::kable(hair_shoe/hair_shoe[3,3],align="c")`
\
\

$p(\text{black hair} \cap \text{sparkly shoes}) \stackrel{?}{=} p(\text{black hair}) \times p(\text{sparkly shoes})$

## Kid example: shared event
...
`r hair_shoe[1,2]/hair_shoe[3,3]` $\stackrel{?}{=}$ `r hair_shoe[1,3]/hair_shoe[3,3]` $\times$ `r hair_shoe[3,2]/hair_shoe[3,3]`
...
`r hair_shoe[1,2]/hair_shoe[3,3]` $\stackrel{?}{=}$ `r hair_shoe[1,3]/hair_shoe[3,3] * hair_shoe[3,2]/hair_shoe[3,3]`
...
You roll a pair of 4-sided dice 1e2 times in a row and record the combination of numbers you get 
