...
The _law of large numbers_ (LLN) states that as the number of samples 
from some process increases, the sample average converges on the 
expected value.
...
## Law of Large Numbers recap:

1. As the number of samples drawn from a distribution 
increases, the sample average converges on the expected value
(and more broadly, the more the sample distribution 
starts to resemble the generating distribution)

2. But if the process is stochastic and has no "memory," 
the probability of subsequent outcomes is independent of 
previous outcomes
...
```{r, out.width="450px",echo=FALSE,fig.align="center"}
	knitr::include_graphics("figs/x-files.jpg")
```

## Parameters - what's the deal? {.build}

A **_statistical parameter_** is a value that tells you something 
about a **_population_**.

- e.g., the _true_ mean height of toddlers in my kid's classroom, 
...
Haphazard and grid searches are inefficient and often impractical (e.g., the mean of a normal distribution).
...
\
\
$\text{max}(f(x)) \text{ happens when } f'(x) = 0 \text{ and } f''(x) < 0$
...
A - `r sum(dbinom(x=n.heads,size=100,prob=0.5))` \
B - `r prod(dbinom(x=n.heads,size=100,prob=0.5))` \
C - `r dbinom(n.heads[1],size=100,prob=0.5)` \
D - `r prod(dnorm(n.heads,mean=50,sd=1))` \
...
```{r}
n.heads <- c(46,47,48)
dbinom(x=n.heads[1],size=100,prob=0.5) * 
dbinom(x=n.heads[2],size=100,prob=0.5) * 
dbinom(x=n.heads[3],size=100,prob=0.5)
prod(dbinom(x=n.heads,size=100,prob=0.5))
```
...
```{r}
prod(dbinom(x=rbinom(10,size=100,prob=0.5),size=100,prob=0.5))
```

What might go wrong here?

```{r}
prod(dbinom(x=rbinom(100,size=100,prob=0.5),size=100,prob=0.5))
```

```{r}
prod(dbinom(x=rbinom(500,size=100,prob=0.5),size=100,prob=0.5))
```
...
<iframe src="figs/log_lady.gif" height="610px" width="800px" align="middle" style="border: none;"></iframe>
