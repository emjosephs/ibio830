...
```{r}
	tmp <- rep("T",100)
	tmp[sample(1:100,43,replace=FALSE)] <- "H"
	tmp
```
...
```{r,fig.width=6.5,fig.height=5,fig.align="center",echo=FALSE}
	plot(0,type='n',xlim=c(0,25),ylim=c(0,0.2),
			xlab="successes",ylab="probability",main="Recreate Me!")
...
- **Discrete** random variables are things that can be counted
	+ heads in a coinflip experiment, fish in a lake
\
\
...
Fun fact: continuous RVs are kind of discrete?!
...
1. A brief history of the distribution (wikipedia is your friend, citations unnecessary unless you make outlandish claims)
